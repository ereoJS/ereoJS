...
> For a full guide on loaders, actions, and the three different approaches to defining them, see [Data Loading](/concepts/data-loading).
...
Cloudflare Pages supports server-side functions via the `functions/` directory. Note that EreoJS already handles API routes via [HTTP method exports](/concepts/data-loading#api-routes-http-method-exports) in your route files — you only need Pages Functions if you want Cloudflare-specific functionality outside of the EreoJS router:
...
- [Data Loading](/concepts/data-loading) — Loaders, actions, and all three definition approaches
- [Deploying to Vercel](/ecosystem/deployment/vercel) — Alternative deployment platform
