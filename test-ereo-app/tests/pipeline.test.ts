...
describe('createPipeline', () => {
  ...
  test('respects dependencies (waterfall when needed)', async () => {
    const executionOrder: string[] = [];
      ...
      loaders: {
        ...
        profile: {
          load: async ({ data }) => {
            ...
            return { userId: (data as any).user.id, bio: 'Hello!' };
          },
        },
      },
      dependencies: {
        profile: ['user'], // profile depends on user
      },
    });
    ...
    // User must complete before profile starts
    expect(executionOrder).toEqual(['user', 'profile']);
    expect(result.data.profile.userId).toBe(1);
  });
  ...
  test('collects errors for loaders without fallback', async () => {
    const pipeline = createPipeline({
      loaders: {
        failing: {
          load: async () => {
    ...
    expect(result.errors.get('failing')?.message).toBe('Something broke');
  });
...
describe('dataSource helpers', () => {
  ...
  test('cachedSource adds cache options', () => {
    ...
    expect(source.tags).toEqual(['posts']);
