...
import type { BuildConfig, FrameworkConfig, Route, Plugin } from '@areo/core';
...
export async function build(options: BuildOptions = {}): Promise<BuildResult> {
  ...
  const publicPath = options.publicPath || '/_areo/';
...
async function buildServer(options: {
  ...
  try {
    const result = await Bun.build({
      ...
      splitting,
      external: ['react', 'react-dom', '@areo/core', '@areo/router', '@areo/render', ...external],
      naming: {
        entry: '[dir]/[name].[ext]',
  ...
  return {
    outputs,
    errors,
    entryFile: '.areo/server/index.js',
    routeModules,
  };
}
...
function generateServerEntry(routes: Route[], root: string): string {
  ...
  return `/**
 * Server Entry - Auto-generated by @areo/bundler
...
async function buildClient(options: {
  ...
  return {
    outputs,
    errors,
    entryFile: '.areo/client/index.js',
    chunks,
  };
}
 ...
 */
function generateDefaultClientEntry(routes: Route[]): string {
  return `/**
 * Client Entry - Auto-generated by @areo/bundler
 * Do not edit this file directly.
 */
...
async function initClient() {
  ...
  const dataScript = document.getElementById('__AREO_DATA__');
  const serverData = dataScript ? JSON.parse(dataScript.textContent || '{}') : {};

  console.log('[Areo] Client hydration initialized');

  // Initialize islands
  const islands = document.querySelectorAll('[data-island]');
  if (islands.length > 0) {
    console.log(\`[Areo] Found \${islands.length} island(s) to hydrate\`);
  }
}
