...
import type { Route, RouteModule } from '@oreo/core';
...
export function generateRouteTypes(
  ...
  lines.push("declare module '@oreo/core' {");
...
export type ParamsFor<T extends RoutePath> =
  T extends keyof import('@oreo/core').RouteTypes
    ? import('@oreo/core').RouteTypes[T]['params']
...
export type LoaderDataFor<T extends RoutePath> =
  T extends keyof import('@oreo/core').RouteTypes
    ? import('@oreo/core').RouteTypes[T]['loader']
    : unknown;
...
export type ActionDataFor<T extends RoutePath> =
  T extends keyof import('@oreo/core').RouteTypes
    ? import('@oreo/core').RouteTypes[T]['action']
    : unknown;
...
export function createTypesPlugin(options: {
  ...
  const { outDir = '.oreo', routesDir = 'app/routes', inferTypes = true, watch = false } = options;
  let routes: Route[] = [];
