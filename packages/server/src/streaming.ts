/**
 * @oreo/server - React Streaming Support
 *
 * Server-side rendering with streaming support for React 18+.
 */

import type { ReactElement } from 'react';
import type { Route, RouteMatch, AppContext, LoaderFunction } from '@oreo/core';
import { serializeLoaderData } from '@oreo/data';
...
export function createShell(options: {
  ...
  const loaderScript = loaderData
    ? `<script>window.__OREO_DATA__=${serializeLoaderData(loaderData)}</script>`
    : '';
