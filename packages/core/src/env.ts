/**
 * @oreo/core - Environment Variable Management
 *
 ...
 * Use these to define your environment variables in oreo.config.ts.
 *
 * @example
 * import { env } from '@oreo/core';
 *
...
export function validateEnv(
  ...
  // Warn about unknown environment variables with OREO_ prefix
  for (const key of Object.keys(rawEnv)) {
    if (key.startsWith('OREO_') && !schema[key]) {
      warnings.push(`Unknown environment variable with OREO_ prefix: ${key}`);
    }
  }
 ...
 * Use after defining env schema in oreo.config.ts.
 *
 * @example
