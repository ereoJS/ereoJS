...
export function validateEnv(
  ...
  // Warn about unknown environment variables with AREO_ prefix
  for (const key of Object.keys(rawEnv)) {
    if (key.startsWith('AREO_') && !schema[key]) {
      warnings.push(`Unknown environment variable with AREO_ prefix: ${key}`);
    }
  }
