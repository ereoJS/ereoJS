...
import { createServer } from '@oreo/server';
...
export async function start(options: StartOptions = {}): Promise<void> {
  ...
  const buildDir = join(root, '.oreo');

  console.log('\n  \x1b[36m⬡\x1b[0m \x1b[1mOreo\x1b[0m Production Server\n');

  // Check if build exists
  const manifestPath = join(buildDir, 'manifest.json');
  if (!(await Bun.file(manifestPath).exists())) {
    console.error('  \x1b[31m✗\x1b[0m No build found. Run `oreo build` first.\n');
    process.exit(1);
  }
